...
export default function AdminReqEditForm(props) {
  ...
  const _onSubmit = (e) => {
    e.preventDefault();
    toast.dismiss();
    if (req.tipe_menu === ''|| req.nama_menu === '' || req.deskripsi_menu === '' || req.harga_menu <= 0 || req.status_menu === '' ) {
      setSubmited(true);
      toast.error('Harap lengkapi pengisian', { containerId: 'B', transition: Zoom });
    }
    else if (isNaN(req.harga_menu)) {
      setSubmited(true);
      toast.error('Harga menu hanya boleh mengandung angka saja', { containerId:"B", transition:Zoom });
    }
    else {
          ...
          <Card body>
            ...
            <Form>              
              ...
              <FormGroup>
                ...
                <Input id = "harga" type = "number" min={1} value = {req.harga_menu} onChange={(e) => changeReq("harga_menu", e.target.value)} invalid={req.harga_menu <= 0 && submited} />
